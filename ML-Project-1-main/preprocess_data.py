...
def preprocess_global(
    ...
    if replace_nan_by is not None:
        ...
        x_train, x_test = replace_nan(x_train, x_test, replacement_value=replace_nan_by)
    else:
        # Remove columns with a high percentage of NaN values
        x_train, x_test = remove_columns_with_high_nan(x_train, x_test, threshold=column_nan_threshold)

        # Remove rows with a high percentage of NaN values
        x_train, y_train = remove_rows_with_high_nan(x_train, y_train, threshold=row_nan_threshold)

        # Fill remaining NaNs in numerical features
        x_train = fill_missing_values(x_train, continuous_threshold)
        x_test = fill_missing_values(x_test, continuous_threshold)

    # Normalize or standardize x_train and x_test with the given method
    x_train, x_test = normalize_or_standardize(x_train, x_test, method=normalization_method)
    ...
    if outliers == "remove":
        ...
        x_train = clip_outliers(x_train, threshold=z_score_threshold)
    # Check for imbalance and balance the data if needed
    ...
    if pca_ratio is not None and pca_ratio > 0 and pca_ratio <= 1:
        # Apply PCA to training data
        x_train, components, train_mean = perform_pca(x_train, ratio=pca_ratio)  
